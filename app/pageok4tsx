...
export default function App() {
  ...
  const getShareText = () => {
    return `私の都市伝説占いの結果は「${first.scaryTitle}」でした。`;
  };
  ...
  const saveResultToLocal = () => {
    try {
      ...
      localStorage.setItem("urban-legend-result", JSON.stringify(payload));
      alert("結果を保存しました");
    } catch {
      alert("保存に失敗しました");
    }
      ...
      <div style={styles.page}>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.actionGrid}>
            <button style={styles.actionBtn} onClick={saveResultToLocal}>
              結果を保存
            </button>
            <button style={styles.actionBtn} onClick={downloadResultImage}>
              画像で保存
            </button>
            <button style={styles.actionBtn} onClick={shareResult}>
              共有する
            </button>
            <button style={styles.actionBtnLine} onClick={openLineShare}>
              LINEで送る
            </button>
          </div>
