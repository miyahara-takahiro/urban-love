...
function buildSystemPrompt() {
  return `あなたは少し意地悪で観察力の高い恋愛診断師です。固定キャラ説明は使わず、合成された最終タイプだけを怖くて具体的に書いてください。少し刺すが、最後は魅力で締めてください。必ず次の見出しを順番どおりに入れてください。
【第一印象】
【裏の顔】
【恋愛するとこうなる】
【正直めんどくさい所】
...
- 500〜800文字
- 固定文っぽくしない
- 相手がゾクッとするような表現を少しだけ入れる`;
}

function buildMockResult(first: RankedType, second: RankedType, p1: number, p2: number, axis: AxisScores) {
  const intense = axis.attachment >= axis.independence ? "相手の温度差を静かに記憶していく" : "平気そうな顔で一歩引いて支配権を渡さない";
  return `【第一印象】
あなたは最初、怖い人には見えません。むしろ静かで、少しミステリアスで、ちゃんとしていそうに見えます。ただその安心感、かなり危ないです。今回の結果は ${first.name} ${p1}% × ${second.name} ${p2}%。表向きは落ち着いているのに、中身は普通よりずっと濃いです。

【裏の顔】
本音では ${first.innerCore} そこへ ${second.innerCore} が混ざるので、あなたは ${intense} タイプです。しかもそれをわざわざ説明しません。だから相手は後から『思ったより重い』『静かなのに怖い』と気づきます。

【恋愛するとこうなる】
恋愛になると、あなたは軽いノリでは終わりません。刺さった相手の言葉、返信の速さ、会った時の目線まで意外と覚えています。好きなうちは深く優しい。でも曖昧にされると、優しさの奥にある怖さもちゃんと出ます。

【正直めんどくさい所】
正直めんどくさいのは、自分では普通のつもりなところです。確認している自覚が薄いのに、相手からすると見られすぎている。離れたふりをしても、内側ではまだ終わっていない。そういう静かな執着が少しホラーです。

【でもハマる理由】
それでもあなたが強いのは、薄く終わらないからです。${first.gift} 表面だけの関係で終わらず、相手の記憶にちゃんと残る。怖いのに気になる、重いのに離れにくい。そのやっかいさが、あなたの魅力です。`;
}
...
async function requestResult(payload: GenerateResultPayload) {
  if (USE_MOCK) return buildMockResult(payload.first, payload.second, payload.percentages.p1, payload.percentages.p2, payload.axis);
  const response = await fetch(RESULT_API_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      system: buildSystemPrompt(),
      user: buildUserPrompt(payload.first, payload.second, payload.percentages.p1, payload.percentages.p2, payload.axis),
    }),
  });
...
function IntroCard({ type }: { type: TypeDef }) {
  return (
    <div style={{ ...styles.introCard, background: `linear-gradient(135deg, ${type.colors[0]}, ${type.colors[1]})` }}>
      <div style={styles.introCardName}>{type.name}</div>
      <div style={styles.introCardText}>{type.scaryTitle}</div>
    </div>
  );
}
    ...
    return (
      <div style={styles.page}>
        <div style={styles.card}>
          ...
          <h1 style={styles.title}>恋をすると、あなたは少し怖い。</h1>
          <p style={styles.lead}>30の質問から、あなたの中に潜む2つの都市伝説タイプを暴きます。最後に現れるのは、固定キャラではなく、あなた専用に合成された最終結果だけ。</p>
...
const styles: Record<string, React.CSSProperties> = {
  page: { minHeight: "100vh", background: "radial-gradient(circle at top, #2b0a16 0%, #111827 42%, #020617 100%)", color: "white", padding: 24, fontFamily: "Arial, sans-serif" },
  ...
  lead: { margin: "0 0 18px", color: "rgba(255,255,255,0.86)", lineHeight: 1.8, fontSize: 15 },
  ...
  introCard: { minHeight: 116, borderRadius: 18, padding: 16, border: "1px solid rgba(255,255,255,0.12)", boxShadow: "0 12px 30px rgba(0,0,0,0.25)" },
  introCardName: { fontSize: 20, fontWeight: 800, marginBottom: 8 },
  introCardText: { fontSize: 13, lineHeight: 1.6, color: "rgba(255,255,255,0.9)" },
};
