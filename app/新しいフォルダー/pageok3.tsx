  ...
  if (viewMode === "intro") {
    return (
      <div style={styles.horrorPage}>
        ...
        <div style={styles.horrorGlowA} />
        <div style={styles.horrorGlowB} />

        <div style={styles.horrorWrap}>
          <div style={styles.horrorTopline}>
            <span style={styles.horrorDot} />
            urban legend.exe / forbidden access
          </div>

          <div style={styles.horrorPanel}>
            <div style={styles.horrorBadge}>都市伝説占い</div>

            <h1 style={styles.horrorTitle}>都市伝説.exe</h1>
            <div style={styles.horrorSubTitle}>あなたの内側に棲む怪異を解析します</div>

            <p style={styles.horrorLead}>
              ...
              都市伝説は、外にいるだけではなく、
              たぶん人の内側にも潜んでいます。
              この占いでは、あなたの内に潜む狂気と執着、
              そして恋をした時に顔を出す危うさを30の質問で暴きます。
            </p>

            <div style={styles.horrorWarningBox}>
              <div style={styles.horrorWarningLabel}>WARNING</div>
              <div style={styles.horrorWarningText}>
                結果には、不穏・執着・恋愛ホラー的な表現が含まれます。
                <br />
                それでも続行する場合のみ、診断を開始してください。
              </div>
            </div>

            <div style={styles.genderRow}>
              <button style={gender === "male" ? styles.genderBtnActive : styles.genderBtn} onClick={() => setGender("male")}>
                男性向け
              </button>
              <button style={gender === "female" ? styles.genderBtnActive : styles.genderBtn} onClick={() => setGender("female")}>
                女性向け
              </button>
              <button style={gender === "other" ? styles.genderBtnActive : styles.genderBtn} onClick={() => setGender("other")}>
                指定なし
              </button>
            </div>

            <div style={styles.horrorIntroGrid}>
            ...
            <div style={styles.horrorFooterNote}>
              there is no guarantee that you will return unchanged
            </div>
          </div>
        </div>
      </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  pageNoise: {
    ...
    inset: 0,
    pointerEvents: "none",
    background: "radial-gradient(circle at 20% 20%, rgba(255,255,255,0.04), transparent 24%), radial-gradient(circle at 80% 30%, rgba(255,255,255,0.03), transparent 20%), radial-gradient(circle at 40% 80%, rgba(255,255,255,0.02), transparent 22%)",
    opacity: 0.35,
  },
  ...
  horrorGlowA: {
    ...
    height: 560,
    borderRadius: "50%",
    background: "radial-gradient(circle, rgba(255,0,60,0.16), transparent 65%)",
    filter: "blur(10px)",
    pointerEvents: "none",
  },
  ...
  horrorGlowB: {
    position: "absolute",
    inset: 0,
    background: "radial-gradient(circle at 20% 20%, rgba(120,0,255,0.08), transparent 28%), radial-gradient(circle at 80% 30%, rgba(255,0,0,0.08), transparent 30%)",
    pointerEvents: "none",
  },
  ...
  horrorTitle: {
    ...
    letterSpacing: 4,
    color: "#fff5f5",
    textShadow: "0 0 8px rgba(255,0,0,0.45), 0 0 24px rgba(255,0,0,0.18)",
  },
